...
const TOOLS = Object.freeze([
  ...
  },
  {
    ...
    description: [
      ...
      'You supply the coordinates, not the developer: pass the `symbol` or `file` the rule is ABOUT',
      'so it can be anchored and can expire when that code changes. A rule true of the whole',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        symbol: {
          type: 'string',
          description: 'The declaration this is about, if any. For a fact/hazard/etc., it expires when that declaration is rewritten or deleted. For an open_loop, it resurfaces when that declaration is visited instead of expiring.',
        },
        file: {
          type: 'string',
          description: 'Repository-relative path this is about, when no single declaration owns it. Same effect as `symbol` for an open_loop: it resurfaces there instead of expiring.',
        },
        repo: {
          type: 'string',
