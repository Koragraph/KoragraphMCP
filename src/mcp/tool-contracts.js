...
const TOOLS = Object.freeze([
  ...
  },
  {
    ...
    description: [
      ...
      'Guess-grade edges (HEURISTIC_CALLS), statistical co-change, and a declaration\'s own',
      'structural membership (DEFINED_IN/CONTAINS — what is inside/around it) are excluded unless',
      'you ask for them; for co-change use changes_with instead. On a CLASS/SERVICE/INTERFACE',
      'symbol, direction "in" also reaches real callers of that type\'s own methods, not just',
      'direct references to the type itself — the common case for "what depends on this class".',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        include_defined_in: {
          type: 'boolean',
          default: false,
          description: 'Include DEFINED_IN/CONTAINS — a declaration\'s own structural membership edges (its file containing it, its class owning it: "what is inside/around this", not "what depends on this"). Off by default so a class\'s member count cannot bury its real callers/callees.',
        },
        depth: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        exclude_non_source: {
          type: 'boolean',
          default: false,
          description: 'Drop callers in non-source files (.md, .mdc, .txt, .rst, .adoc) — IDE rule/template/doc files that reference a changed name but are not a call site. Off by default: results already rank source ahead of these, this removes them outright.',
        },
        project_id: PROJECT_ARG,
        detail: DETAIL_ARG,
      },
